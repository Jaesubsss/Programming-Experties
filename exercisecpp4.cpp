...
int main(int argc, char **argv) {
  popl::OptionParser pexapp("pexban application\nUsage: pexban [options] number\nOptions"); // creating parser
  auto help = pexapp.add<popl::Switch>("h", "help", "produce help message");
  auto interactive = pexapp.add<popl::Switch>("i", "interactive", "start interactive mode");
  pexapp.parse(argc, argv); // argc는 인자의 개수, argv는 인자의 배열
  ...
  if (interactive->is_set()) {
    ...
    pex.box("all");
    std::cout << "Run: " << argv[0] << " --help for help.\n";
  }
  return 0;
}
